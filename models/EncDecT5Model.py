...
from transformers import AutoModel, AutoTokenizer, AutoConfig, MarianTokenizer, T5ForConditionalGeneration, T5Tokenizer
import json
...
class EncDecT5Model(nn.Module):
    """Huggingface AutoModel to generate token embeddings.
    Loads the correct class, e.g. BERT / RoBERTa etc.
    """
    def __init__(self, model_name_or_path: str, max_seq_length: int = 128, task="translation", model_args: Dict = {}, cache_dir: Optional[str] = None, freeze_encoder=False, source_language=None,  target_language=None):
        super(EncDecT5Model, self).__init__()
        self.config_keys = ['max_seq_length']
        self.max_seq_length = max_seq_length

        config = AutoConfig.from_pretrained(model_name_or_path,**model_args, cache_dir=cache_dir)
        self.model = T5ForConditionalGeneration.from_pretrained(model_name_or_path, config=config, cache_dir=cache_dir)
        ...
        if task == "translation" and source_language is not None and target_language is not None:
            self.translation_sentence = "translate "+source_language+" to "+target_language
    ...
    def load(input_path: str, task, freeze_encoder):
        ...
        return EncDecT5Model(model_name_or_path=input_path, task=task, freeze_encoder=freeze_encoder) #, **config)
